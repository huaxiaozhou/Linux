...
static inline _syscall0 (int, fork)		// 这是unistd.h 中的内嵌宏代码。以嵌入汇编的形式调用
...
#define BCD_TO_BIN(val) ((val)=((val)&15) + ((val)>>4)*10)	// 将BCD 码转换成数字。
     ...
     {
       struct tm time;		// 时间结构tm 定义在include/time.h 中。
...
void main (void)
{
  ...
  // 下面过程通过在堆栈中设置的参数，利用中断返回指令切换到任务0执行。
  move_to_user_mode ();		// 移到用户模式下执行。 （include/asm/system.h，第1 行）
  if (!fork ())
...
void init (void)
{
  ...
  // 系统设备。该函数是用25 行上的宏定义的，对应函数是sys_setup()，在kernel/blk_drv/hd.c,71
  //行。
    setup ((void *) &drive_info);
    // 下面以读写访问方式打开设备“/dev/tty0”，它对应终端控制台。
    // 由于这是第一次打开文件操作，因此产生的文件句柄号（文件描述符）肯定是0。该句柄是UNIX 类
  ...
  _exit (0);			/* NOTE! _exit, not exit() *//* 注意！是_exit()，不是exit() */
  // _exit()和exit()都用于正常终止一个函数。但_exit()直接是一个sys_exit 系统调用，而exit()则
