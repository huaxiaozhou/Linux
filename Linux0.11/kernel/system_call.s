...
sa_mask = 4 				// 信号量屏蔽码
sa_flags = 8 				// 信号集。
sa_restorer = 12 		// 返回恢复执行的地址位置。恢复函数指针，参见kernel/signal.c。
...
reschedule:
	...
	jmp _schedule		#在执行完schedule()后最终会返回到ret_from_sys_call 处继续执行。
...
# 如果该任务在就绪状态但其时间片已用完（counter=0）,counter[??]值等于0，则也去执行调度程序。
