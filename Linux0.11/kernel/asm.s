/*
asm.s 汇编程序中包括大部分CPU 探测到的异常故障处理的底层代码，也包括数学协处理器（FPU）
的异常处理。该程序与kernel/traps.c 程序有着密切的关系。该程序的主要处理方式是在中断处理程序中
调用相应的C 函数程序，显示出错位置和出错号，然后退出中断。
eax = -1
系统中断调用(eax=调用号)
ebx,ecx,edx 中放有调用参数
调用号超范围?
...
mov %dx,%ds
mov %dx,%es
mov %dx,%fs	# 下行上的'*'号表示是绝对调用操作数，与程序指针PC 无关。
