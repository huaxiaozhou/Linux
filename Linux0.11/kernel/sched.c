...
{												// 定义任务联合(任务结构成员和stack 字符数组程序成员)。
  struct task_struct task;	// 因为一个任务的数据结构与其内核态堆栈放在同一内存页中，所以
...
repeat:current->state = TASK_INTERRUPTIBLE;
  ...
  // 等待队列的任务均被抹掉了。当然，同时也需删除192 行上的语句。参见图4.3。
  *p = NULL;
  if (tmp)
    tmp->state = 0;
}
 ...
 */
// 这里用于软驱定时处理的代码是201–262 行。在阅读这段代码之前请先看一下块设备一章中有关
// 软盘驱动程序（floppy.c）后面的说明。或者到阅读软盘块设备驱动程序时在来看这段代码。
// 其中时间单位：1 个滴答 = 1/100 秒。
// 下面数组存放等待软驱马达启动到正常转速的进程指针。数组索引0-3 分别对应软驱A-D。
static struct task_struct *wait_motor[4] = { NULL, NULL, NULL, NULL };
// 下面叔祖分别存放各软驱马达启动所需要的滴答数。程序中默认启动时间为50 个滴答（0.5 秒）。
static int mon_timer[4] = { 0, 0, 0, 0 };
// 下面数组分别存放各软驱在马达停转之前需维持的时间。程序中设定为10000 个滴答（100 秒）。
static int moff_timer[4] = { 0, 0, 0, 0 };
// 对应软驱控制器中当前数字输出寄存器。该寄存器每位的定义如下：
// 位7-4：分别控制驱动器D-A 马达的启动。1 - 启动；0 - 关闭。
// 位3 ：1 - 允许DMA 和中断请求；0 - 禁止DMA 和中断请求。
// 位2 ：1 - 启动软盘控制器；0 - 复位软盘控制器。
// 位1-0：00 - 11，用于选择控制的软驱A-D。
unsigned char current_DOR = 0x0C;	// 这里设置初值为：允许DMA 和中断请求、启动FDC。
																		// 数字输出寄存器(初值：允许DMA 和请求中断、启动FDC)。
...
{
  extern unsigned char selected;	// 选中软驱标志(kernel/blk_drv/floppy.c,122)。
  unsigned char mask = 0x10 << nr;	// 所选软驱对应数字输出寄存器中启动马达比特位。
																	  // mask 高4 位是各软驱启动马达标志。
  if (nr > 3)
    panic ("floppy_on: nr>3");	// 最多4 个软驱。
  // 首先预先设置好指定软驱nr 停转之前需要经过的时间（100 秒）。然后取当前DOR 寄存器值到
  // 临时变量mask 中，并把指定软驱的马达启动标志置位。
  ...
  // 要求启动的马达还没有启动，则置相应软驱的马达启动定时器值(HZ/2 = 0.5 秒或50 个滴答)。若
  // 已经启动，则再设置启动定时为2 个滴答，能满足下面do_floppy_timer()中先递减后判断的要求。
  // 执行本次定时代码的要求即可。此后更新当前数字输出寄存器变量current_DOR。
  if (mask != current_DOR)
    {
      outb (mask, FD_DOR);
...
// 置关闭相应软驱马达停转定时器（3 秒）。
// 若不使用该函数明确关闭指定的软驱马达，则在马达开启100 秒之后也会被关闭。
void
...
// 系统调用功能 - 设置报警定时时间值(秒)。
// 如果参数seconds>0，则设置该新的定时值并返回原定时值。否则返回0。
int
