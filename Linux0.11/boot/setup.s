...
INITSEG  = 0x9000	! 原来bootsect 所处的段。
SYSSEG   = 0x1000	! system 在0x10000(64k)处。
...
! ok，整个读磁盘过程都正常，现在将光标位置保存以备今后使用。
	...
	mov	ax,#INITSEG	! 将ds 置成#INITSEG(0x9000)。这已经在bootsect 程序中
										! 设置过，但是现在是setup 程序，Linus 觉得需要再重新
										! 设置一下。
	mov	ds,ax
	mov	ah,#0x03			! BIOS 中断0x10 的读光标功能号 ah = 0x03
										! 输入：bh = 页号
...
! 硬盘的表，0x90090 处存放第2 个硬盘的表。
	...
	rep							!重复，根据cx
	movsb									!MOVSB（MOVe String Byte）：即字符串传送指令，
												!这条指令按字节传送数据。通过SI和DI这两个寄存器控制字符串
												!的源地址和目标地址
! Get hd1 data
	mov	ax,#0x0000
	mov	ds,ax
...
! 的内存数据块(512k)，整块地向内存低端移动了0x10000（64k）的位置。
	...
	cld			! 'direction'=0, movs moves forward
					!清方向标志位，也就是使DF的值为0，在执行串操作时，使地址按递增的方式变化，
					!这样便于调整相关段的的当前指针
do_move:
...
!! 所以我们就必须重新对8259 中断控制器进行编程，这一点都没劲。
	...
	mov	al,#0x11				! 0x11 表示初始化命令开始，是ICW1 命令字，表示边
										! 沿触发、多片8259 级连、最后要发送ICW4 命令字。
	out	#0x20,al			! 发送到8259A 主芯片。

	! 下面定义的两个字是直接使用机器码表示的两条相对跳转指令，起延时作用。
! 0xeb 是直接近跳转指令的操作码，带1 个字节的相对位移值。因此跳转范围是-127 到127。CPU 通过
! 把这个相对位移值加到EIP 寄存器中就形成一个新的有效地址。此时EIP 指向下一条被执行的指令。
! 执行时所花费的CPU 时钟周期数是7 至10 个。0x00eb 表示跳转值是0 的一条指令，因此还是直接
! 执行下一条指令。这两条指令共可提供14-20 个CPU 时钟周期的延迟时间。在as86 中没有表示相应
! 指令的助记符，因此Linus 在setup.s 等一些汇编程序中就直接使用机器码来表示这种指令。另外，
! 每个空操作指令NOP 的时钟周期数是3 个，因此若要达到相同的延迟效果就需要6 至7 个NOP 指令。
...
! 控制寄存器CR0，其比特位0 置1 将导致CPU 工作在保护模式。
	...
	lmsw	ax								! 就这样加载机器状态字
													!置处理器状态字。但是只有操作数的低4位被存入CR0，
													!只有PE,MP,EM和TS被改写，CR0其他位不受影响
	jmpi	 0,8									! 跳转至cs 段8，偏移0 处。
