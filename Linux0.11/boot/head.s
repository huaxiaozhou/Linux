/*
* head.s 含有32 位启动代码。请注意代码中赋值是从左到右
...
# 的第2 项）=0x10，并将堆栈放置在stack_start 指向的user_stack 数组区，然后使用本程序
# 后面定义的新中断描述符表和全局段描述表。新全局段描述表中初始内容与setup.s 中的基本
# 一样，仅段限长从8MB 修改成了16MB。stack_start 定义在kernel/sched.c，69 行。它是指向
# user_stack 数组末端的一个长指针。
	...
	movl $0x10,%eax				# reload all the segment registers
													...
													# CS 代码段寄存器已经在setup_gdt 中重新加载过了。
	mov %ax,%ds						# after changing gdt. CS was already
	mov %ax,%es						# reloaded in 'setup_gdt'
	mov %ax,%fs
	mov %ax,%gs
# 由于段描述符中的段限长从setup.s 程序的8MB 改成了本程序设置的16MB（见setup.s 行208-216
# 和本程序后面的行235-236），因此这里再次对所有段寄存器执行加载操作是必须的。
# 另外，通过使用bochs 跟踪观察，如果不对CS 再次执行加载，那么在执行到行26 时CS 代码段不可见
# 部分中的限长还是8MB。这样看来应该重新加载CS，但在实际机器上测试结果表明CS 已经加载过了。
	lss _stack_start,%esp
# 下面几 行用于测试A20 地址线是否已经开启。采用的方法是向内存地址0x000000 处写入任意
...
rp_sidt:
	movl %eax,(%edi)				# 将哑中断门描述符存入表中。
													#“-4(%ebp)”,其表示的内存地址为(%ebp寄存器中的内容-4)得到的值
													#28（%edi）中的28表示偏移量，即整个地址为%edi里的地址加上28个字节
	movl %edx,4(%edi)
...
# 前面3 个入栈0 值应该分别是envp、argv 指针和argc 值，但main()没有用到。
...
* 对于那些有多于16Mb 内存的家伙 – 真是太幸运了，我还没有，为什么你会有☺。代码就在
* 这里，对它进行修改吧。（实际上，这并不太困难的。通常只需修改一些常数等。我把它设置
* 为16Mb，因为我的机器再怎么扩充甚至不能超过这个界限（当然，我的机器是很便宜的☺）。
* 我已经通过设置某类标志来给出需要改动的地方（搜索“16Mb”），但我不能保证作这些
* 改动就行了􀀯）。
*/
# 在内存物理地址0x0 处开始存放1 页页目录表和4 页页表。页目录表是系统所有进程公用的，而
# 这里的4 页页表则是属于内核专用。对于新的进程，系统会在主内存区为其申请页面存放页表。
# 1 页内存长度是4096 字节。
